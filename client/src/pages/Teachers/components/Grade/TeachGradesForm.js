...
import TeachersGrades from "../../TeachersGrades";
...
import GradesTable from "./TeachGradesTable";
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        ...
        const subAPI = await axiosPrivate.get("/api/subjects", {
          headers: { "Content-Type": "application/json" },
          withCredentials: true,
        });
        if (subAPI.status === 200) {
          const json = await subAPI.data;
...
export default TeachGradesForm;
